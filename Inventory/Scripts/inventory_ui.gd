...
func _process(delta):
	...
	if Input.is_action_just_pressed("ui_accept") and container.get_children().size() != 0 and canvas.visible:
		...
		if more_info_panel.visible:
			if more_info_panel.selected_option == more_info_panel.options.DROP:
				inventory.remove_item(inventory.playerData.inventory[selected_item])
				more_info_panel.visible = false
				render_items()
				render_selected()  
			elif more_info_panel.selected_option == more_info_panel.options.USE:
				if inventory.playerData.inventory[selected_item].action != "":
					items_actions.call_deferred(inventory.playerData.inventory[selected_item].action)
		else:
	...
	if Input.is_action_just_pressed("ui_down") and selected_item < inventory.playerData.inventory.size()-1:
		selected_item += 1
...
func render_selected():
	if selected_item > inventory.playerData.inventory.size()-1 or selected_item < 0:
		selected_item = 0
		
	for i in range(container.get_children().size()):
		if selected_item == i:
			more_info_panel.item_name = inventory.playerData.inventory[i].name
			more_info_panel.item_description = inventory.playerData.inventory[i].description
...
func render_items():
	if inventory.playerData.inventory.size() == 0:
		more_info_panel.visible = false
		
	for child in container.get_children():
		container.remove_child(child)
	for i in range(inventory.playerData.inventory.size()):
		var ui_item = preload("res://Inventory/inventory_ui_item.tscn")
		var ui_item_instance = ui_item.instantiate()
		ui_item_instance.item_name = inventory.playerData.inventory[i].name
		ui_item_instance.item_count = inventory.playerData.inventory[i].quantity
